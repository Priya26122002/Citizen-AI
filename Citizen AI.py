...
Original file is located at
    https://colab.research.google.com/drive/15ZbPzVlAzJ9VgR60k5gWsyYzzMhL9F2F
"""
...
esponse = tokenizer.decode(outputs[0], skip_special_tokens=True)
esponse = response.replace(prompt, "").strip()
return response
...
eturn generate_response(prompt, max_lenth=1000)

itizen_interaction(query):
rompt = f"As a government assistant, provide accurate and helpful information about the following citizen query related to public services, government policies, or civic issues:\n\nQuery: {query}"
eturn generate_response(prompt, max_length=1000)

ate Gradio interface
gr. Blocks() as app:
r.Markdown("# City Analysis & Citizen Services AI")
...
ith gr.tabs():
   with gr.TabItem("City Analysis"):
       with gr.Row():
           ...
           with gr.Column():
               city_output = gr.Textbox(lebel="City Analysis (Crime Index & Accidents)", lines=15)
